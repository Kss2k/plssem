...
pls_msg_note <- function(..., showheader = TRUE, footer = "") {
...
pls_msg_warn <- function(..., footer = "", immediate. = FALSE) {
  wat <- unlist(list(...), use.names = FALSE)
  msg <- pls_msg(wat)
  if (footer != "") msg <- paste(msg, footer, sep = "\n")
...
# Warn immediately
pls_msg_warn_immediate <- function(..., footer = "") {
  pls_msg_warn(..., footer = "", immediate. = TRUE)
}

# Displays a message with header and formatted as
# above via R function 'stop()'.
pls_msg_stop <- function(..., footer = "") {
  wat <- unlist(list(...), use.names = FALSE)
  msg <- pls_msg(wat)
  if (footer != "") msg <- paste(msg, footer, sep = "\n")
...
pls_msg_fixme <- function(...) {
  wat <- c("FIXME: ", unlist(list(...), use.names = FALSE))
  stop(pls_msg(wat), call. = FALSE, domain = NA)
}

# subroutine for above functions
pls_msg <- function(wat, txt_width = getOption("width", 80L),
  ...
  if (showheader) {
    ignore_in_stack <- c(
      ...
      "^message", "\\.local", "^[0-9]+$" # anonymous functions
    )
    sc <- sys.calls()
    sc_i <- length(sc)
    sc_naam <- ""
