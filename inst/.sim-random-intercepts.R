...
gamma_x <- c(gamma_f_x1, gamma_f_x2, gamma_f_x3)  # (x1,x2,x3)
gamma_w <- c(gamma_f_w1, gamma_f_w2)              # (w1,w2)

# Covariance matrix for (x1,x2,x3)
Sigma_x <- diag(rep(var_ov, 3))
Sigma_x[1,2] <- Sigma_x[2,1] <- cov_x1_x2
Sigma_x[1,3] <- Sigma_x[3,1] <- cov_x1_x3
Sigma_x[2,3] <- Sigma_x[3,2] <- cov_x2_x3

# Covariance matrix for (w1,w2)
Sigma_w <- matrix(
  c(var_ov,    cov_w1_w2,
    cov_w1_w2, var_ov),
  nrow = 2, byrow = TRUE
...
# Fixed part variance contributions
var_fixed_x <- as.numeric(t(gamma_x) %*% Sigma_x %*% gamma_x)
var_fixed_w <- as.numeric(t(gamma_w) %*% Sigma_w %*% gamma_w)

# Random intercept + residual variances
var_u <- var_beta_f   # Var(u)

# Expected variance of f
var_f_proj <- var_fixed_x + var_fixed_w + var_u
zeta_f <- 1 - var_f_proj


lhs <- c(rep("f", 5), "f~1")
rhs <- c("x1", "x2", "x3", "w1", "w2", "f~1")
op  <- c(rep("~", 5), "~~")
parTable.true <- data.frame(
  ...
  est.true = c(gamma_f_x1, gamma_f_x2, gamma_f_x3, gamma_f_w1, gamma_f_w2, var_beta_f)
)
...
sim_cont_data <- function(N = n, K = k) {
  ...
  x2 <- x2
  x3 <- x3
  SW <- diag(rep(var_ov, 2))
  SW[1, 2] <- SW[2, 1] <- cov_w1_w2

  W <- rmvnorm(K, mean = c(0, 0), sigma = SW)
  w1 <- W[cluster, 1]
  w2 <- W[cluster, 2]
  ...
  X <- matrix(c(rep(1, N), x1, x2, x3, w1, w2), nrow = N, ncol = 6, byrow = FALSE)
  Z <- matrix(0L, nrow = N, ncol = Kc) # Here we only have random intercepts
  
  for (ki in seq_len(Kc))
    Z[cluster==ki, ki] <- 1L

  Beta <- c(beta_f, gamma_f_x1, gamma_f_x2, gamma_f_x3, gamma_f_w1, gamma_f_w2)
  u <- rnorm(Kc, mean = 0, sd = sqrt(var_beta_f))
  e <- rnorm(N, mean = 0, sd = sqrt(zeta_f))
...
for (cond in names(list_thresholds)) {
  ...
  for (ncat in categories) {
    ...
    for (i in seq_len(R)) {
      ...
      ordered <- setdiff(colnames(data_cat_i), "cluster")
