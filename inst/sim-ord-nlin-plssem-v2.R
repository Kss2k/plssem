...
createFile <- function() {
  con <- file(filename, "w")
  writeLines(paste0(c("id", "cond", "ncat", "seed", "method", pars), collapse = ";"), con)
  close(con)
}

writeLinesToFile <- function(lines) {
  con <- file(filename, "a+")
  ...
  for (ncat in categories) {
    ...
    for (i in seq_len(R)) {
      ...
      fitted_i <- list(
        ...
        # plsc.ord.pj= get_output(
        #   expr = pls(model, data_cat_i, ordered = ordered, bootstrap = FALSE, mc.polyak.juditsky = TRUE,
        #              mc.tol = 1e-5, mc.min.iter = 500, mc.max.iter = 1000, mc.reps = NROW(data_cat_i)),
        #   method = "MC-OrdPLSc (PJ)", id = id, cond = cond, ncat = ncat,
        #   parfun = plssem::parameter_estimates
        # ) #,
